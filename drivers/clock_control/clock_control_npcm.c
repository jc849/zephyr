...
/* I3C clock divider */
#define I3C_TC_DIV_VAL  (DT_PROP(DT_NODELABEL(pcc), i3c_tc_prescaler) - 1)
...
#define NPCM_CLOCK_REG_OFFSET(n)     (((n) >> NPCM_CLOCK_PWDWN_GROUP_OFFSET) & 0xf)
					      ...
					      clock_control_subsys_t sub_system, uint32_t *rate)
{
	ARG_UNUSED(dev);
	uint32_t clk_cfg = (uint32_t)sub_system;
	uint8_t bus_id = (clk_cfg >> NPCM_CLOCK_BUS_OFFSET);
