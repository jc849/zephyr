...
#define DT_DRV_COMPAT nuvoton_npcm4xx_ethernet

#define LOG_MODULE_NAME eth_npcm4xx
...
#include "eth_npcm4xx.h"
#include "eth.h"

#define ETH_NPCM4XX_MTU 1500
...
#define ETH_NPCM4XX_REG_BASE    ((struct emac_reg *)(DT_INST_REG_ADDR(0)))
...
struct eth_npcm4xx_dev_cfg {
	void (*config_func)(void);
};

/* Device run time data */
struct eth_npcm4xx_dev_data {
	struct net_if *iface;
	uint8_t mac_addr[6];
	...
	K_KERNEL_STACK_MEMBER(rx_thread_stack,
		CONFIG_ETH_NPCM4XX_RX_THREAD_STACK_SIZE);
	struct k_thread rx_thread;
	bool link_up;
};

#define __eth_npcm4xx_desc __aligned(4)
#define __eth_npcm4xx_buf  __aligned(4)
/* Ethernet Rx DMA Descriptors */
static struct ETH_DMA_DESCRIPTOR  DMARxDscrTab[ETH_RXBUFNB] __eth_npcm4xx_desc;
/* Ethernet Tx DMA Descriptors */
static struct ETH_DMA_DESCRIPTOR  DMATxDscrTab[ETH_TXBUFNB] __eth_npcm4xx_desc;
/* Ethernet Receive Buffers */
static uint8_t Rx_Buff[ETH_RXBUFNB][ETH_RX_BUF_SIZE] __eth_npcm4xx_buf;
/* Ethernet Transmit Buffers */
static uint8_t Tx_Buff[ETH_TXBUFNB][ETH_TX_BUF_SIZE] __eth_npcm4xx_buf;
...
{
	struct emac_reg *const emac_regs = ETH_NPCM4XX_REG_BASE;
	uint32_t tmpreg = 0;
	...
	tmpreg = (emac_regs->MACMIIAR & (0x0F << NPCM4XX_MACMIIAR_CR));
	/* Prepare the MII register address value */
	/* Set the PHY device address */
	tmpreg |= (((uint32_t)heth->InitParm.PhyAddress << 11) & (0x1F << NPCM4XX_MACMIIAR_PA));
	/* Set the PHY register address */
	tmpreg |= (((uint32_t)PHYReg << 6) & (0x1F << NPCM4XX_MACMIIAR_MR));
	/* Set the write mode */
	tmpreg |= BIT(NPCM4XX_MACMIIAR_MW);
	/* Set the MII Busy bit */
	tmpreg |= BIT(NPCM4XX_MACMIIAR_MB);
	...
	while ((tmpreg & BIT(NPCM4XX_MACMIIAR_MB)) == BIT(NPCM4XX_MACMIIAR_MB)) {
...
enum ETH_RET_STATUS ETH_TransmitFrame(struct ETH_HANDLE_TYPE *heth, uint32_t FrameLength)
{
	...
	if ((emac_regs->DMASR & BIT(NPCM4XX_DMASR_TBUS)) != 0) {
		/* Clear TBUS ETHERNET DMA flag */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_TBUS);
...
{
	struct emac_reg *const emac_regs = ETH_NPCM4XX_REG_BASE;
	struct eth_npcm4xx_dev_data *dev_data = dev->data;
	struct ETH_HANDLE_TYPE *heth;
	uint8_t *dma_buffer;
	int res;
	size_t total_len;
	...
	if ((emac_regs->DMASR & BIT(NPCM4XX_DMASR_TUS)) != 0) {
		/* Clear TUS ETHERNET DMA flag */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_TUS);
		/* Resume DMA transmission*/
		emac_regs->DMATPDR = 0;
		res = -EIO;
		goto error;
...
static struct net_if *get_iface(struct eth_npcm4xx_dev_data *ctx,
				uint16_t vlan_tag)
{
	ARG_UNUSED(vlan_tag);
...
{
	struct emac_reg *const emac_regs = ETH_NPCM4XX_REG_BASE;
	struct eth_npcm4xx_dev_data *dev_data;
	struct ETH_HANDLE_TYPE *heth;
...
release_desc:
	 ...
	 */
	if ((emac_regs->DMASR & BIT(NPCM4XX_DMASR_RBUS)) != 0) {
		/* Clear RBUS ETHERNET DMA flag */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_RBUS);
...
static void rx_thread(void *arg1, void *unused1, void *unused2)
{
	...
	while (1) {
		res = k_sem_take(&dev_data->rx_int_sem,
			K_MSEC(CONFIG_ETH_NPCM4XX_CARRIER_CHECK_RX_IDLE_TIMEOUT_MS));
		if (res == 0) {
...
static void eth_isr(const struct device *dev)
{
	...
	/* Frame received */
	if (sts & BIT(NPCM4XX_DMASR_RS)) {
		/* Clear the Eth DMA Rx IT pending bits */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_RS);
	...
	/* Frame transmitted */
	if (sts & BIT(NPCM4XX_DMASR_TS)) {
		/* Clear the Eth DMA Rx IT pending bits */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_TS);

	}
	/* Normal Interrupt Summary  */
	if (sts & BIT(NPCM4XX_DMASR_NIS)) {
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_NIS);
	}

	/* Abnormal Interrupt Summary */
	if (sts & BIT(NPCM4XX_DMASR_AIS)) {
		/* Clear the ETH DMA Error flags */
		emac_regs->DMASR = BIT(NPCM4XX_DMASR_AIS);
	}
}

#if defined(CONFIG_ETH_NPCM4XX_RANDOM_MAC)
...
void __SetSMIClock(void)
{
	...
	clk_cfg.bus = NPCM4XX_CLOCK_BUS_CORE;

	/* Clock Range (1 MHz ~ 2.5 MHz) */
	value = (emac_regs->MACMIIAR & (0x0F << NPCM4XX_MACMIIAR_CR));
	...
	if (core_clk < 10000000) {
		value |= (0x08 << NPCM4XX_MACMIIAR_CR); /* DIV 4 */
	} else if ((core_clk >= 10000000) && (core_clk < 20000000)) {
		value |= (0x0A << NPCM4XX_MACMIIAR_CR); /* DIV 8 */
	} else if ((core_clk >= 20000000) && (core_clk <= 35000000)) {
		value |= (0x02 << NPCM4XX_MACMIIAR_CR); /* DIV 16 */
	} else if ((core_clk >= 35000000) && (core_clk < 60000000)) {
		value |= (0x03 << NPCM4XX_MACMIIAR_CR); /* DIV 26 */
	} else if ((core_clk >= 60000000) && (core_clk < 100000000)) {
		value |= (0x00 << NPCM4XX_MACMIIAR_CR); /* DIV 42 */
	}
...
void ETH_MACAddressConfig(uint32_t MacAddr, uint8_t *Addr, uint8_t IsSrcAddr, uint8_t MacAddrMsk)
{
	...
	if (MacAddr >= 1) {
		addr_H |= BIT(NPCM4XX_MACA1HR_AE);
		if (IsSrcAddr) {
			addr_H |= BIT(NPCM4XX_MACA1HR_SA);
		}
		if (MacAddrMsk) {
			addr_H |= ((MacAddrMsk & 0x3F) << NPCM4XX_MACA1HR_MBC);
		}
	}
...
void ETH_MACDMAConfig(struct ETH_HANDLE_TYPE *heth, uint32_t err)
{
	...
	if (heth->InitParm.ChecksumMode == ETH_CHECKSUM_BY_HARDWARE) {
		reg |= BIT(NPCM4XX_MACCR_IPCO);
	} else {
		reg &= ~BIT(NPCM4XX_MACCR_IPCO);
	}

	/* Retry disable */
	reg |= BIT(NPCM4XX_MACCR_RD);
	emac_regs->MACCR = reg;

	/*----------------------- ETHERNET MACFFR Configuration ---------------------------*/
	/* forwards all control frames to application except Pause frame. */
	emac_regs->MACFFR = BIT(NPCM4XX_MACFFR_PCF_ForwardExcptPause);
	...
	emac_regs->MACFCR |= BIT(NPCM4XX_MACFCR_ZQPD);
	...
	emac_regs->DMAOMR = (BIT(NPCM4XX_DMAOMR_RSF) | BIT(NPCM4XX_DMAOMR_TSF));
	/*----------------------- ETHERNET DMABMR Configuration ------------------*/
	emac_regs->DMABMR = (0x80000 | 0x400 | BIT(NPCM4XX_DMABMR_EDE));

	if ((heth->InitParm).RxMode == ETH_RXINTERRUPT_MODE) {
		/* Enable the Ethernet Rx Interrupt */
		emac_regs->DMAIER |= (BIT(NPCM4XX_DMAIER_NISE) | BIT(NPCM4XX_DMAIER_RIE));
	}
...
enum ETH_RET_STATUS ETH_Start(struct ETH_HANDLE_TYPE *heth)
{
	...
	emac_regs->MACCR |= BIT(NPCM4XX_MACCR_TE);
	/* Enable the MAC reception */
	emac_regs->MACCR |= BIT(NPCM4XX_MACCR_RE);
	/* Set the Flush Transmit FIFO bit */
	emac_regs->DMAOMR |= BIT(NPCM4XX_DMAOMR_FTF);
	/* Enable the DMA transmission */
	emac_regs->DMAOMR |= BIT(NPCM4XX_DMAOMR_ST);
	/* Enable the DMA reception */
	emac_regs->DMAOMR |= BIT(NPCM4XX_DMAOMR_SR);
...
static int eth_initialize(const struct device *dev)
{
	...
	/* EMAC Software reset */
	RegSetBit(emac_regs->DMABMR, BIT(NPCM4XX_DMABMR_SR));
	timeout = 0;
	while ((emac_regs->DMABMR & BIT(NPCM4XX_DMABMR_SR)) != 0) {
		timeout++;
		if (timeout > 0xFFFFFF) {
			return -ENOTSUP;
		}
	...
	k_thread_create(&dev_data->rx_thread, dev_data->rx_thread_stack,
			...
			K_PRIO_COOP(CONFIG_ETH_NPCM4XX_RX_THREAD_PRIO),
			0, K_NO_WAIT);
...
static void eth_iface_init(struct net_if *iface)
{
	const struct device *dev;
	struct eth_npcm4xx_dev_data *dev_data;
	const struct eth_npcm4xx_dev_cfg *dev_cfg;
...
static enum ethernet_hw_caps eth_npcm4xx_get_capabilities(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static int eth_npcm4xx_set_config(const struct device *dev,
				    ...
				    const struct ethernet_config *config)
{
	...
	case ETHERNET_CONFIG_TYPE_PROMISC_MODE:
		emac_regs->MACFFR |= BIT(NPCM4XX_MACFFR_PM);
		return 0;
	default:
		break;
	}
...
static const struct ethernet_api eth_api = {
	...
	.get_capabilities = eth_npcm4xx_get_capabilities,
	.set_config = eth_npcm4xx_set_config,
	.send = eth_tx,
};
...
static const struct eth_npcm4xx_dev_cfg eth0_config = {
	.config_func = eth0_irq_config,
};

static struct eth_npcm4xx_dev_data eth0_data = {
	.mac_addr = {
		NUVOTON_OUI_B0,
		NUVOTON_OUI_B1,
		NUVOTON_OUI_B2,
#if !defined(CONFIG_ETH_NPCM4XX_RANDOM_MAC)
		CONFIG_ETH_NPCM4XX_MAC3,
		CONFIG_ETH_NPCM4XX_MAC4,
		CONFIG_ETH_NPCM4XX_MAC5
	#endif
	},
	.heth = {
		.InitParm = {
...
ETH_NET_DEVICE_DT_INST_DEFINE(0, eth_initialize,
		    ...
		    CONFIG_ETH_INIT_PRIORITY, &eth_api, ETH_NPCM4XX_MTU);
